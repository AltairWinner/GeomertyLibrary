...
--Подробно:
--В задании требуется сделать так, чтобы продукту могло соответствовать много категорий, а в одной категории могло быть много продуктов. 
--Это отношение "многие ко многим", которое моделируется с помощью третьей, вспомогательной таблицы. Поэтому было создано 3 таблицы с помощью следующих команд:
...
--Поэтому мы собираем данные следующим образом: к таблице продуктов products прикрепляем с помощью LEFT JOIN пары (т.е. таблицу products_categories), 
--которые надо будет вывести, сравнивая записи по product_id
--В получившейся таблице оказываются все требуемые нам ряды, то есть и пары продукт-категория, и продукты без пар (где в категории просто окажется NULL). 
--Но в этой таблице нет названий категорий. Ещё раз используем LEFT JOIN, используя category_id для присоединения таблицы categories.
